...
// --- Utility Functions (extracted to src/utils/ and src/services/) ---
import { formatSize, getInitials } from './utils/formatters.js';
import { getFileType, fileToBase64 } from './utils/fileHelpers.js';
import { callGeminiAPI, callImagenAPI } from './services/aiService.js';
...
const MOCK_FILES_INITIAL = [
  { id: '1', parentId: null, name: 'Documents', type: 'folder', size: '--', date: '2023-10-24', starred: true, isTrashed: false },
  { id: '2', parentId: null, name: 'Images', type: 'folder', size: '--', date: '2023-11-02', starred: false, isTrashed: false },
  { id: '3', parentId: null, name: 'Work Projects', type: 'folder', size: '--', date: '2023-09-15', starred: true, isTrashed: false },
  { id: '4', parentId: null, name: 'Budget_2024.xlsx', type: 'file', fileType: 'excel', size: '24 KB', date: '2023-12-01', starred: false, isTrashed: false },
  { id: '5', parentId: null, name: 'Resume.pdf', type: 'file', fileType: 'pdf', size: '450 KB', date: '2024-01-10', starred: true, isTrashed: false },
  { id: '6', parentId: '1', name: 'Notes.txt', type: 'file', fileType: 'text', size: '2 KB', date: '2023-10-25', starred: false, isTrashed: false },
  { id: '7', parentId: '1', name: 'Report.docx', type: 'file', fileType: 'word', size: '1.2 MB', date: '2023-10-26', starred: false, isTrashed: false },
  { id: '8', parentId: '2', name: 'Vacation', type: 'folder', size: '--', date: '2023-11-05', starred: false, isTrashed: false },
  { id: '9', parentId: '2', name: 'Profile_Pic.jpg', type: 'file', fileType: 'image', size: '2.4 MB', date: '2023-11-02', starred: false },
  { id: '10', parentId: '8', name: 'Beach.png', type: 'file', fileType: 'image', size: '3.1 MB', date: '2023-11-05', starred: false },
];
...
export default function CloudStorageApp() {
  ...
  const handleMoveFiles = () => {
      const filesToMove = selectedIds.size > 0 
  ...
  const handleAIAnalyzeImage = async (file) => {
      ...
      if (file.rawFile) {
          try {
              ...
              const response = await callGeminiAPI("Analyze this image. Describe what you see and suggest 3 relevant tags.", base64, file.rawFile.type);
              setAiResponse(response);
          } catch (e) {
  ...
  const handleGlobalAskAI = async () => {
      ...
      const fileList = files.map(f => `- ${f.name} (${f.fileType}, ${f.date})`).join('\n');
      const prompt = `
  ...
  const displayedItems = useMemo(() => {
    ...
    if (activeSidebar === 'gallery') {
        if (galleryTab === 'albums' && currentAlbum) {
            return items.filter(item => !item.isTrashed && currentAlbum.fileIds.includes(item.id));
        }
        return items.filter(item => !item.isTrashed && (item.fileType === 'image' || item.fileType === 'video'));
    }
  ...
  }, [files, currentFolder, activeSidebar, searchQuery, galleryTab, currentAlbum]);
  ...
  const availableFolders = useMemo(() => {
      return files.filter(f => f.type === 'folder' && !f.isTrashed && !selectedIds.has(f.id));
  }, [files, selectedIds]);
  ...
  const borderCol = userProfile.darkMode ? 'border-gray-700' : 'border-gray-200';
  const hoverBg = userProfile.darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50';
  const itemBg = userProfile.darkMode ? 'bg-gray-800' : 'bg-white';
  const itemBorder = userProfile.darkMode ? 'border-gray-700' : 'border-gray-100';
  ...
  return (
    <div className={`flex h-screen w-full font-sans overflow-hidden transition-colors duration-300 ${bgMain} ${textMain}`}>
      ...
      {/* Sidebar */}
      <aside className={`w-64 flex-shrink-0 border-r flex flex-col transition-all duration-300 hidden md:flex ${bgPanel} ${borderCol}`}>
        <div className="p-4 flex items-center space-x-3 text-indigo-600">
          <Cloud className="w-8 h-8 fill-current" />
          <span className={`text-xl font-bold tracking-tight ${textMain}`}>CloudDrive</span>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-2 space-y-1">
          {SIDEBAR_ITEMS.map((item) => (
            ...
            >
              <item.icon className={`w-5 h-5 ${activeSidebar === item.id ? 'text-indigo-500' : 'text-gray-500'}`} />
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        {/* Header */}
        <header className={`h-16 border-b flex items-center justify-between px-4 lg:px-6 flex-shrink-0 ${bgPanel} ${borderCol}`}>
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            <div className={`flex p-1 rounded-lg ${userProfile.darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                <button 
            ...
            <div className={`h-6 w-px mx-2 hidden sm:block ${userProfile.darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}></div>
            ...
            <div 
                ...
                style={{ backgroundColor: userProfile.avatar ? 'transparent' : '#e0e7ff', color: userProfile.avatar ? 'transparent' : '#4338ca' }}
            >
              {userProfile.avatar ? (
        ...
        {/* Toolbar & Breadcrumbs */}
        <div className={`px-4 lg:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between border-b gap-4 min-h-[73px] ${borderCol}`}>
          <div className="flex-1 min-w-0">
             {activeSidebar === 'my-files' && !searchQuery ? (
               <Breadcrumbs path={path} onNavigate={handleNavigate} darkMode={userProfile.darkMode} />
             ) : (
               <div className="flex items-center gap-4">
                 {activeSidebar === 'gallery' && galleryTab === 'albums' && currentAlbum ? (
                     <div className="flex items-center gap-2">
                        ...
                        <h2 className={`text-xl font-bold capitalize tracking-tight ${textMain}`}>
                            {currentAlbum.name}
                        </h2>
                     </div>
                 ) : (
                    <div className="flex items-center gap-4">
                         <h2 className={`text-xl font-bold capitalize tracking-tight ${textMain}`}>
                            {searchQuery ? 'Search Results' : activeSidebar.replace('-', ' ')}
                        </h2>
                        
                        {activeSidebar === 'gallery' && (
                            <div className={`flex rounded-lg p-1 text-sm font-medium ${userProfile.darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                                <button 
          ...
          <div className="flex items-center space-x-2">
             {activeSidebar === 'gallery' && galleryTab === 'albums' && !currentAlbum && (
                  <button onClick={() => setIsCreateAlbumOpen(true)} className="flex items-center space-x-2 px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">
             ...
             {selectedIds.size > 0 && (
                ...
                {activeSidebar === 'trash' ? (
                    <>
                    <button onClick={restoreFromTrash} className="p-1.5 hover:bg-indigo-500 rounded-lg transition-colors" title="Restore">
                        <RefreshCcw className="w-5 h-5" />
                    </button>
                    <div className="h-4 w-px bg-indigo-400 mx-1"></div>
                    <button onClick={deletePermanently} className="p-1.5 hover:bg-indigo-500 rounded-lg transition-colors" title="Delete Permanently">
                    ...
                    <>
                        ...
                        <button onClick={openMoveModal} className="p-1.5 hover:bg-indigo-500 rounded-lg transition-colors" title="Move to">
                            <FolderInput className="w-5 h-5" />
                        </button>
                        <button onClick={() => setIsAddToAlbumOpen(true)} className="p-1.5 hover:bg-indigo-500 rounded-lg transition-colors" title="Add to Album">
                    ...
                    <div className="h-4 w-px bg-indigo-400 mx-1"></div>
                    <button onClick={moveToTrash} className="p-1.5 hover:bg-indigo-500 rounded-lg transition-colors" title="Move to Trash">
                        <Trash2 className="w-5 h-5" />
                    </button>
                    </>
                )}
        ...
        <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-4 lg:p-6" onClick={() => setSelectedIds(new Set())}>
                ...
                {activeSidebar === 'gallery' && galleryTab === 'albums' && !currentAlbum ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                        {albums.map(album => {
                            ...
                            return (
                                <div key={album.id} onClick={() => handleAlbumClick(album)} className="group cursor-pointer">
                                    <div className={`aspect-square rounded-2xl overflow-hidden mb-3 border-2 ${itemBorder} relative`}>
                                        {coverFile ? (
                                            <img src={coverFile.previewUrl} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                        ) : (
                                            <div className={`w-full h-full flex items-center justify-center ${userProfile.darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                                                <ImageIcon className="w-12 h-12 text-gray-300" />
                                            </div>
                                        )}
                                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
                                    </div>
                                    <h3 className={`font-semibold truncate ${textMain}`}>{album.name}</h3>
                                    <p className={`text-xs ${textSec}`}>{album.fileIds.length} items</p>
                                </div>
                            )
                        })}
                         <div 
                         ...
                         >
                             ...
                             <span className={`text-sm font-medium ${textSec}`}>Create Album</span>
                         </div>
                    </div>
                ) : (
                    displayedItems.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-gray-400">
                        <div className={`w-32 h-32 rounded-full flex items-center justify-center mb-6 ${userProfile.darkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                            {activeSidebar === 'trash' ? <Trash2 className="w-12 h-12 text-gray-300" /> : <Folder className="w-12 h-12 text-gray-300" />}
                        </div>
                        <p className={`text-xl font-semibold ${textSec}`}>
                            {activeSidebar === 'trash' ? 'Trash is empty' : currentAlbum ? 'Album is empty' : 'No files found'}
                        </p>
                        </div>
                        ...
                        {displayedItems.map((item) => (
                            ...
                            >
                            <div className={`relative aspect-[4/3] mb-3 rounded-xl flex items-center justify-center overflow-hidden ${userProfile.darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
                                <FileIcon 
                                    type={item.type} 
                                    fileType={item.fileType} 
                                    className={item.type === 'folder' || !item.previewUrl ? "w-12 h-12" : "w-full h-full"} 
                                    previewUrl={item.previewUrl}
                                />
                                
                                <div className={`absolute inset-0 bg-black/5 transition-opacity flex justify-between items-start p-2 ${selectedIds.has(item.id) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                                    <button 
                                    ...
                                    >
                                        {selectedIds.has(item.id) ? <CheckCircle2 className="w-5 h-5 fill-current" /> : <div className="w-5 h-5 rounded-full border-2 border-gray-300" />}
                                    </button>
                            ...
                            <div className="flex-1 min-w-0">
                                <h3 className={`text-sm font-semibold truncate ${textMain}`} title={item.name}>
                                    {item.name}
                                </h3>
                                <div className="flex items-center justify-between mt-1">
                                    <p className={`text-xs ${textSec}`}>{item.type === 'folder' ? item.date : item.size}</p>
                                </div>
                            </div>
                            </div>
                        ))}
                        ...
                        <div className={`border rounded-xl overflow-hidden shadow-sm ${bgPanel} ${borderCol}`}>
                            <table className="min-w-full divide-y divide-gray-200/50">
                            <thead className={userProfile.darkMode ? 'bg-gray-800' : 'bg-gray-50'}>
                                <tr>
                                <th scope="col" className="px-6 py-4 w-10"></th>
                                <th scope="col" className={`px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider ${textSec}`}>Name</th>
                                <th scope="col" className={`px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider hidden sm:table-cell ${textSec}`}>Date Modified</th>
                                <th scope="col" className={`px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider hidden md:table-cell ${textSec}`}>Size</th>
                                <th scope="col" className="relative px-6 py-4"></th>
                                </tr>
                            </thead>
                            <tbody className={`divide-y ${userProfile.darkMode ? 'divide-gray-700' : 'divide-gray-100'}`}>
                                {displayedItems.map((item) => (
                                <tr 
                                    key={item.id}
                                    onClick={(e) => {
                                    ...
                                    selectedIds.has(item.id) ? 'bg-indigo-500/10' : hoverBg
                                    }`}
                                >
                                    <td className="px-6 py-4 whitespace-nowrap">
                                    <button onClick={(e) => toggleSelection(e, item.id)} className="focus:outline-none">
                                        {selectedIds.has(item.id) ? (
                                    ...
                                    <div className="flex items-center">
                                        <div className={`flex-shrink-0 h-10 w-10 rounded-lg flex items-center justify-center ${userProfile.darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
                                            <FileIcon type={item.type} fileType={item.fileType} className="w-6 h-6" previewUrl={item.previewUrl} />
                                        </div>
                                        <div className="ml-4">
                                            <div className={`text-sm font-semibold transition-colors ${textMain}`}>{item.name}</div>
                                            <div className={`text-xs ${textSec}`}>{item.type}</div>
                                        </div>
                                    </div>
                                    </td>
                                    <td className={`px-6 py-4 whitespace-nowrap text-sm hidden sm:table-cell ${textSec}`}>{item.date}</td>
                                    <td className={`px-6 py-4 whitespace-nowrap text-sm hidden md:table-cell ${textSec}`}>{item.size}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        {!item.isTrashed && (
                                            <button onClick={(e) => toggleStar(e, item.id)} className="p-1 hover:bg-gray-500/20 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Star className={`w-4 h-4 ${item.starred ? 'text-yellow-400 fill-current' : 'text-gray-400'}`} />
                                            </button>
                                        )}
                                    </td>
                                </tr>
            ...
            {activeFile && previewPaneOpen && (
                <div className={`w-80 border-l flex flex-col transition-all duration-300 flex-shrink-0 animate-in slide-in-from-right-10 ${bgPanel} ${borderCol}`}>
                    <div className="p-4 flex items-center justify-between border-b border-gray-100 dark:border-gray-700">
                        <span className={`font-semibold ${textMain}`}>{activeFile.name}</span>
                        <button onClick={() => setPreviewPaneOpen(false)} className={`p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 ${textSec}`}>
                            <X className="w-4 h-4" />
                        </button>
                    </div>
                    <div className="p-4 flex-1 overflow-y-auto">
                        <div className={`aspect-video rounded-xl flex items-center justify-center mb-6 overflow-hidden ${userProfile.darkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
                            {activeFile.previewUrl ? (
                                <img src={activeFile.previewUrl} className="w-full h-full object-contain" />
                            ) : (
                                <FileIcon type={activeFile.type} fileType={activeFile.fileType} className="w-20 h-20 opacity-50" />
                            )}
                        </div>
                        
                        <h3 className={`font-bold text-lg mb-1 break-words ${textMain}`}>{activeFile.name}</h3>
                        <p className={`text-sm mb-6 ${textSec}`}>{activeFile.type} • {activeFile.size}</p>

                        <div className="space-y-4">
                            <div className={`p-4 rounded-xl border ${userProfile.darkMode ? 'bg-indigo-900/20 border-indigo-800' : 'bg-indigo-50 border-indigo-100'}`}>
                                <div className="flex items-center space-x-2 mb-3">
                                    <Sparkles className="w-4 h-4 text-indigo-500" />
                                    <h4 className={`text-sm font-semibold text-indigo-600 dark:text-indigo-400`}>AI Assistant</h4>
                                </div>
                                ...
                                {isAiLoading ? (
                                    ...
                                    <div className="animate-in fade-in">
                                        <div className={`text-sm leading-relaxed mb-3 p-3 rounded-lg ${userProfile.darkMode ? 'bg-gray-800 text-gray-200' : 'bg-white text-gray-700'}`}>
                                    ...
                                    <div className="space-y-2">
                                        {activeFile.type !== 'folder' && (activeFile.fileType === 'text' || activeFile.fileType === 'pdf' || activeFile.fileType === 'word' || activeFile.fileType === 'excel') && (
                                            <button 
                                        ...
                                        {activeFile.type !== 'folder' && activeFile.fileType !== 'image' && activeFile.fileType !== 'text' && (
                                            <button 
                                                onClick={() => handleAISummary(activeFile)}
                                                className={`w-full flex items-center justify-center space-x-2 py-2 rounded-lg text-sm font-medium transition-colors ${userProfile.darkMode ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-50'}`}
                                            >
                            ...
                            <div>
                                <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${textSec}`}>Information</h4>
                                <div className="space-y-3 text-sm">
                                    <div className="flex justify-between">
                                        <span className={textSec}>Type</span>
                                        <span className={textMain}>{activeFile.fileType || 'Folder'}</span>
                                    </div>
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className={textMain}>{currentFolder ? 'Folder' : 'My Files'}</span>
                                    </div>
                                </div>
                            </div>
                            
                            <div className="pt-4 border-t dark:border-gray-700 space-y-2">
                                <button onClick={() => downloadFile(activeFile)} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-gray-700 ${textMain}`}>
                                    <Download className="w-4 h-4" />
                                    <span>Download</span>
                                </button>
                                <button onClick={() => setIsAddToAlbumOpen(true)} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-gray-700 ${textMain}`}>
                                    <Album className="w-4 h-4" />
                                    <span>Add to Album</span>
                                </button>
                                <button onClick={openMoveModal} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors hover:bg-gray-100 dark:hover:bg-gray-700 ${textMain}`}>
                                    <FolderInput className="w-4 h-4" />
                                    <span>Move to...</span>
                                </button>
                                {!activeFile.isTrashed ? (
                                    <button onClick={moveToTrash} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors hover:bg-red-50 text-red-600 dark:hover:bg-red-900/30`}>
                                        <Trash2 className="w-4 h-4" />
                                        <span>Move to Trash</span>
                                    </button>
                                ) : (
                                    <button onClick={() => {
                                        setFiles(files.map(f => f.id === activeFile.id ? { ...f, isTrashed: false } : f));
      ...
      {isDeleteConfirmOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className={`rounded-2xl shadow-2xl w-full max-w-sm p-6 ${bgPanel}`}>
                <div className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mb-4 text-red-600 mx-auto">
                    <AlertTriangle className="w-6 h-6" />
                </div>
                <h3 className={`text-xl font-bold text-center mb-2 ${textMain}`}>Empty Trash?</h3>
      ...
      {isMoveModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className={`rounded-2xl shadow-2xl w-full max-w-md p-6 ${bgPanel} flex flex-col max-h-[80vh]`}>
                <div className="flex justify-between items-center mb-4 border-b pb-3 border-gray-100 dark:border-gray-700">
                    ...
                    <button onClick={() => setIsMoveModalOpen(false)} className={`p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 ${textSec}`}>
                        <X className="w-5 h-5" />
                    </button>
                </div>
      ...
      {isGenerateModalOpen && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
              <div className={`rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col ${bgPanel}`}>
                  <div className={`px-6 py-4 border-b flex justify-between items-center ${borderCol}`}>
                      <h2 className={`text-xl font-bold flex items-center gap-2 ${textMain}`}>
                          <Wand2 className="w-6 h-6 text-purple-500" />
                          AI Studio
                      </h2>
                      <button onClick={() => setIsGenerateModalOpen(false)} className={`p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 ${textSec}`}>
                          <X className="w-6 h-6" />
                      </button>
                  </div>
                  ...
                  <div className="p-6">
                      <div className="mb-4">
                          <label className={`block text-sm font-medium mb-2 ${textMain}`}>
                              {generationType === 'image' ? 'Describe the image you want to create' : 'Describe the video you want to generate'}
                          </label>
                          <textarea 
                              placeholder={generationType === 'image' ? "A futuristic city with flying cars, neon lights, 4k render..." : "A time-lapse of a blooming flower in a garden..."}
      ...
      {isAiSearchOpen && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
              <div className={`rounded-2xl shadow-2xl w-full max-w-lg p-6 flex flex-col max-h-[85vh] ${bgPanel}`}>
                  <div className="flex justify-between items-center mb-4 border-b pb-3 border-gray-100 dark:border-gray-700">
                      <div className="flex items-center space-x-2">
                          <Sparkles className="w-6 h-6 text-indigo-500" />
                          <h3 className={`text-xl font-bold ${textMain}`}>Smart Search</h3>
                      </div>
                      <button onClick={() => setIsAiSearchOpen(false)} className={`p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 ${textSec}`}>
                          <X className="w-6 h-6" />
                      </button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto mb-4">
                      {aiSearchResponse ? (
                          <div className={`p-4 rounded-xl text-sm leading-relaxed ${userProfile.darkMode ? 'bg-indigo-900/30 text-gray-200' : 'bg-indigo-50 text-gray-800'}`}>
                          ...
                          <div className={`text-center py-10 ${textSec}`}>
                              ...
                              <p className="text-xs mt-2 opacity-60">"Where are my invoices?"</p>
                              <p className="text-xs opacity-60">"Summarize my recent work"</p>
                          </div>
                      )}
                  </div>
                  ...
                  <div className="relative">
                      ...
                      >
                          {isAiSearchLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
                      </button>
                  </div>
              </div>
          </div>
      ...
      {isSettingsOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
              <div className={`rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] ${bgPanel}`}>
                  {/* Modal Header */}
                  <div className={`px-6 py-4 border-b flex justify-between items-center ${borderCol}`}>
                      <h2 className={`text-xl font-bold flex items-center gap-2 ${textMain}`}>
                      ...
                      <button onClick={() => setIsSettingsOpen(false)} className={`p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 ${textSec}`}>
                          <X className="w-6 h-6" />
                      </button>
                  </div>

                  <div className="flex flex-1 overflow-hidden">
                      <div className={`w-1/3 border-r p-3 space-y-1 ${userProfile.darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-100'}`}>
                          {[
                      ...
                      <div className="flex-1 p-6 overflow-y-auto">
                              ...
                              <div className="space-y-6">
                                  <div className="flex items-center space-x-4 mb-6">
                                      <div className="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 text-2xl font-bold overflow-hidden">
                                          {settingsForm.avatar ? (
                                              <img src={settingsForm.avatar} alt="Profile" className="w-full h-full object-cover" />
                                          ) : (
                                      ...
                                      <div>
                                          ...
                                          />
                                          <p className={`text-xs mt-1 ${textSec}`}>Allowed *.jpeg, *.jpg, *.png</p>
                                      </div>
                                  </div>

                                  <div className="space-y-4">
                                      <div>
                                          <label className={`block text-sm font-medium mb-1 ${textMain}`}>Display Name</label>
                                          <input 
                                              type="text" 
                                      ...
                                      <div>
                                          <label className={`block text-sm font-medium mb-1 ${textMain}`}>Email Address</label>
                                          <input 
                                              type="email" 
                          ...
                          {activeSettingsTab === 'preferences' && (
                              <div className="space-y-6">
                                  <div>
                                      <h3 className={`text-sm font-medium mb-4 ${textMain}`}>Notifications</h3>
                                      <div className="space-y-4">
                                          <div className="flex items-center justify-between">
                                              <div>
                                                  <p className={`text-sm font-medium ${textMain}`}>Email Notifications</p>
                                                  <p className={`text-xs ${textSec}`}>Receive emails about account activity</p>
                                              </div>
                                              <button 
                                              ...
                                              >
                                                  <span className={`w-4 h-4 bg-white rounded-full shadow transform transition-transform duration-200 ${settingsForm.notifications ? 'translate-x-6' : 'translate-x-1'}`} />
                                              </button>
                                          </div>
                                      </div>
                                  </div>
                                  ...
                                  <div className={`pt-6 border-t ${borderCol}`}>
                                      <h3 className={`text-sm font-medium mb-4 ${textMain}`}>Appearance</h3>
                                      <div className="flex items-center justify-between">
                                          <div className="flex items-center space-x-2">
                                              <Moon className={`w-5 h-5 ${textSec}`} />
                                              <div>
                                                  <p className={`text-sm font-medium ${textMain}`}>Dark Mode</p>
                                                  <p className={`text-xs ${textSec}`}>Reduce eye strain</p>
                                              </div>
                                          </div>
                                          <button 
                                              onClick={() => setSettingsForm({...settingsForm, darkMode: !settingsForm.darkMode})}
                                          ...
                                          >
                                              <span className={`w-4 h-4 bg-white rounded-full shadow transform transition-transform duration-200 ${settingsForm.darkMode ? 'translate-x-6' : 'translate-x-1'}`} />
                                          </button>
                                      </div>
                                  </div>
                              </div>
                  ...
                  <div className={`px-6 py-4 border-t flex justify-end space-x-3 ${userProfile.darkMode ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-100'}`}>
                      <button 
      ...
      {isUploadModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className={`rounded-2xl shadow-2xl w-full max-w-md p-6 relative ${bgPanel}`}>
             <button onClick={() => setIsUploadModalOpen(false)} className={`absolute top-4 right-4 p-1 rounded-full ${textSec} hover:bg-gray-100`}>
                <X className="w-6 h-6" />
              </button>
            ...
            >
              ...
              <p className={`text-sm font-medium ${textMain}`}>Click to upload local files</p>
              <input type="file" ref={fileInputRef} className="hidden" multiple onChange={handleFileUpload} />
            </div>

            {/* Cloud Import Options */}
            <div>
                <p className={`text-xs font-semibold uppercase tracking-wider mb-3 ${textSec}`}>Or import from cloud</p>
                <div className="grid grid-cols-3 gap-3">
                    ...
                    >
                        ...
                        <span className={`text-xs font-medium ${textMain}`}>Google Drive</span>
                    </button>
                    ...
                    >
                        ...
                        <span className={`text-xs font-medium ${textMain}`}>OneDrive</span>
                    </button>
                    ...
                    >
                        ...
                        <span className={`text-xs font-medium ${textMain}`}>Dropbox</span>
                    </button>
                </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className={`rounded-2xl shadow-2xl w-full max-w-sm p-6 ${bgPanel}`}>
                ...
                <div className="flex justify-end space-x-3 mt-6">
                    <button onClick={() => setIsCreateFolderOpen(false)} className={`px-4 py-2 rounded-lg font-medium ${textSec} hover:bg-gray-100`}>Cancel</button>
                    <button onClick={createFolder} className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium">Create</button>
                </div>
            </div>
        </div>
      )}
      ...
      {isCreateAlbumOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className={`rounded-2xl shadow-2xl w-full max-w-sm p-6 ${bgPanel}`}>
                ...
                />
                <div className="flex justify-end space-x-3 mt-6">
                    <button onClick={() => setIsCreateAlbumOpen(false)} className={`px-4 py-2 rounded-lg font-medium ${textSec} hover:bg-gray-100`}>Cancel</button>
                    <button onClick={createAlbum} className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium">Create</button>
                </div>
            </div>
        </div>
      )}
      ...
      {isAddToAlbumOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className={`rounded-2xl shadow-2xl w-full max-w-sm p-6 ${bgPanel}`}>
                <h3 className={`text-lg font-bold mb-4 ${textMain}`}>Add to Album</h3>
                <div className="space-y-2 max-h-60 overflow-y-auto">
                    {albums.map(album => (
                        ...
                        >
                            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${userProfile.darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                    ...
                    <button
                        onClick={() => { setIsAddToAlbumOpen(false); setIsCreateAlbumOpen(true); }}
                ...
                <div className="flex justify-end space-x-3 mt-6">
                    <button onClick={() => setIsAddToAlbumOpen(false)} className={`px-4 py-2 rounded-lg font-medium ${textSec} hover:bg-gray-100`}>Cancel</button>
                </div>
            </div>
        </div>
      )}
      ...
      {renameItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
             <div className={`rounded-2xl shadow-2xl w-full max-w-sm p-6 ${bgPanel}`}>
                ...
                />
                <div className="flex justify-end space-x-3 mt-6">
                    <button onClick={() => setRenameItem(null)} className={`px-4 py-2 rounded-lg font-medium ${textSec} hover:bg-gray-100`}>Cancel</button>
                    <button onClick={handleRename} className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium">Save</button>
                </div>
            </div>
        </div>
      )}
      ...
      {previewFile && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/95 backdrop-blur-sm p-4 sm:p-8" onClick={() => setPreviewFile(null)}>
            <div className="max-w-6xl max-h-[95vh] w-full flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center text-white mb-4">
                    <div className="flex items-center space-x-3">
                         <div className="p-2 bg-white/10 rounded-lg">
                            <FileIcon type={previewFile.type} fileType={previewFile.fileType} className="w-6 h-6 text-white" />
                         </div>
                         <div>
                             <h3 className="font-semibold text-lg">{previewFile.name}</h3>
                             <p className="text-sm text-gray-400">{previewFile.size} • {previewFile.date}</p>
                         </div>
                    </div>
                    <div className="flex items-center space-x-2">
                        <button 
                        ...
                        <button onClick={() => setPreviewFile(null)} className="p-2 hover:bg-white/10 rounded-full text-white transition-colors">
                            <X className="w-6 h-6" />
                        </button>
                    </div>
                </div>
                ...
                <div className="flex-1 bg-black/50 rounded-2xl flex items-center justify-center overflow-hidden border border-white/10 shadow-2xl relative min-h-[400px]">
                    {previewFile.fileType === 'image' && previewFile.previewUrl ? (
                        ...
                        <div className="text-center text-gray-300">
                             ...
                             <p className="text-sm text-gray-500 mt-2">Download the file to view content</p>
                        </div>
                    )}
                </div>
            </div>
